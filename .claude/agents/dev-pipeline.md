...
1. `git checkout -b <issue-number>-<slug> master`
2. `git commit --allow-empty -m "wip: <title>"` → `git push -u origin` → `gh pr create --draft --repo muness/atlas`
3. **Then** spawn `oh-solution-space` agent. Bias against Local Optimum solutions — if it involves intricate coordination or new abstractions, step up to Reframe/Redesign.
...
1. Spawn `oh-execute` agent with the session file as context
2. Push commits to PR branch.
...
Ship posts each step's findings as PR comments — that's the auditable quality record.
