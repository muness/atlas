...
>
> **Jump to:** [SQLAlchemy + PostgreSQL usage](#sqlalchemy--postgresql-usage)
...
```bash
# Build from source
cd cmd/atlas && go build -o ~/go/bin/atlas .
...
- **Triggers are owned by functions** — if you change both a function and its trigger in one edit, both appear in the same migration (functions first, then triggers)
